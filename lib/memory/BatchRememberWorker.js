...
export class BatchRememberWorker {
   ...
   * 성공/실패와 무관하게 마지막에 ack(LREM)로 processing 항목을 정리한다.
  ...
  async _processJobEnvelope(envelope) {
    ...
    try {
      ...
      if (retryCount < limit) {
        ...
        await pushToQueue(queueName, { jobId, params, retryCount: retryCount + 1 });
      } else {
        logError(`[BatchRememberWorker] Job ${jobId} exhausted retries, dead-lettering: ${err.message}`);
        await pushToQueueDead(queueName, { jobId, params, retryCount, lastError: err.message });
      }
    } finally {
      await ackQueueItem(queueName, raw);
    }
  }
}
